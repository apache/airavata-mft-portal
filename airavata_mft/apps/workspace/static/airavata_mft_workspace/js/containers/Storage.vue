			...
			<div class="btn-toolbar">
				...
				<div class="btn-group mr-4" role="group">
					<button class="btn btn-sm btn-outline-secondary list">
						<i class="fa fa-bars" v-on:click="layout = 'list'" v-bind:class="{ 'active': layout == 'list'}"></i>
					</button>
					<button class="btn btn-sm btn-outline-secondary list">
						<i class="fa fa-th" v-on:click="layout = 'grid'" v-bind:class="{ 'active': layout == 'grid'}"></i>
					</button>
				</div>
				<div class="btn-group mr-4 new-unit" role="group">
					<button class="btn btn-info btn-sm">
						<i class="fa fa-plus plus-icon"></i>Add new unit
					</button>
				</div>
			</div>
		</div>
		...
		<table v-if="layout === 'list'" class="table table-hover main-table " >
			...
			<tbody>
                ...
                :key="storage.storageId" @click="showDescription(storage)">
					<th scope="row">
						<i class="fa fa-database unit-name-icon" aria-hidden="true"></i>{{storage.name}}
					</th>
					<td>{{storage.size}}</td>
					<td>
